...
parser.add_argument('--model', default='Deit_drop_Tiny', type=str, metavar='MODEL',
...
parser.add_argument('--loss_add', action='store_true', default=False)
...
def main():
    ...
    try:  # train the model
        for epoch in range(start_epoch, num_epochs):
            ...
            if saver is not None and args.no_save_model == False:
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if not args.loss_add:
                ...
                x, head = model(input)
                loss_x = loss_fn(x, target)
                loss_head = loss_fn(head, target)
                loss = loss_x + loss_head
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    'Speed: {speed.val:.5f} ({speed.avg:.5f})  '
                    'Loss: {loss.val:>7.4f} ({loss.avg:>6.4f})  '
                    'Acc@1: {top1.val:>7.4f} ({top1.avg:>7.4f})  '
                    'Acc@5: {top5.val:>7.4f} ({top5.avg:>7.4f})'.format(
                        log_name, batch_idx, last_idx, speed=speed_m, batch_time=batch_time_m,
